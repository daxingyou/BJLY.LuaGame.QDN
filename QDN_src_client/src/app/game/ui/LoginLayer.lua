...
function LoginLayer:initUI()
    ...
    if g_GameConfig.development then
        local content = cc.ui.UIPushButton.new()
            :setButtonSize(200, 40)
            :setButtonLabel(cc.ui.UILabel.new({text = "选择服务器", size = 24, color = display.COLOR_BLUE}))
            :onButtonClicked(function(event)
                 local serverSelected = require("app.game.debug.ServerSelected").new()
                 serverSelected:addTo(self)
            end)
        content:setTouchSwallowEnabled(false)
        content:addTo(self)
        content:setPosition(display.width-100, 100)
        print("heihei")
    end
